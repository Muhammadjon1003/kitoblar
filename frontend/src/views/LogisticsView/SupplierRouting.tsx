...
interface SupplierRoutingProps {
  hideOrderingSection?: boolean;
}

export default function SupplierRouting({ hideOrderingSection = false }: SupplierRoutingProps) {
  const {
    orders,
    groups,
    getStudentName,
  ...
  return (
    <div className="flex-1 overflow-y-auto px-3.5 sm:px-7 py-4 sm:py-6 space-y-6 bg-slate-50">
      ...
      {/* ── Sub-Navbar Header Switcher (Hidden in CRM) ── */}
      {!hideOrderingSection && (
      ...
      {activeTab === 'active' || hideOrderingSection ? (
        <>
          {/* ── To'langan buyurtmalar (Hidden in CRM) ── */}
          {!hideOrderingSection && (
